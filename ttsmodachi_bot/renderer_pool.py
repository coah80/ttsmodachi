...
class WorkerLane:
    ...
    def render(self, payload: RenderPayload, timeout: float) -> dict[str, Any]:
        ...
        if self.startup_failed:
            self.restart()
            if not self.ready.wait(timeout=min(timeout, 15.0)):
                raise RuntimeError(f"Renderer worker {self.spec.name} is not ready after restart")
            if self.startup_failed:
    ...
    def _apply_state(self, state: object) -> None:
        ...
        if "last_recovery_reason" in state:
            self.last_recovery_reason = (
                str(state["last_recovery_reason"]) if state.get("last_recovery_reason") else None
            )
        if "last_error" in state:
            self.last_error = str(state["last_error"]) if state.get("last_error") else None
...
class RendererPool:
    ...
    def render(self, payload: RenderPayload) -> dict[str, Any]:
        rom = payload.voice.rom()
        lanes = self.lanes_by_rom.get(rom)
        if not lanes:
            raise RuntimeError(f"No renderer worker configured for ROM {rom}")

        attempted: set[WorkerLane] = set()
        errors: list[str] = []
        for _ in range(len(lanes)):
            lane = self._choose_lane(rom, lanes, attempted)
            if lane is None:
                break
            attempted.add(lane)
            try:
                ...
                errors.append(f"{lane.spec.name}: {error}")
                if lane.startup_failed or self._is_worker_recoverable_error(error):
                    lane.restart()

        detail = " | ".join(errors[-3:]) if errors else "no workers were available"
        raise RuntimeError(f"All renderer workers failed for ROM {rom}: {detail}")

    def _choose_lane(
        self,
        rom: str,
        lanes: list[WorkerLane],
        attempted: set[WorkerLane],
    ) -> WorkerLane | None:
        available = [
            lane
            for lane in lanes
            if lane not in attempted
            and not lane.startup_failed
            and (lane.process is None or lane.process.is_alive())
        ]
        if not available:
            available = [lane for lane in lanes if lane not in attempted]
        if not available:
            return None

        lowest_pending = min(lane.pending_count() for lane in available)
        candidates = {lane for lane in available if lane.pending_count() == lowest_pending}
        warm_candidates = {lane for lane in candidates if not lane.paused}
        if warm_candidates:
            candidates = warm_candidates
        start = self.next_index.get(rom, 0) % len(lanes)
        for offset in range(len(lanes)):
            index = (start + offset) % len(lanes)
            lane = lanes[index]
            if lane in candidates:
                self.next_index[rom] = (index + 1) % len(lanes)
                return lane
        return None

    @staticmethod
    def _is_worker_recoverable_error(error: Exception) -> bool:
